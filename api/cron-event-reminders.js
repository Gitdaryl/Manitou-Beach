...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const event of events) {
      ...
      // Send reminder to each attendee (email + SMS)
    ...
    console.log(`Cron event reminders: ${totalSent} emails + ${smsSent} SMS for ${events.length} events`);
    return res.status(200).json({ sent: totalSent, smsSent, events: events.length });
  } catch (err) {
