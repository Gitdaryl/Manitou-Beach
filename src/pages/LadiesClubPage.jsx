...
function LadiesClubHero() {
  ...
  return (
    <section style={{ position: "relative", overflow: "hidden", textAlign: "center", minHeight: "92vh", display: "flex", alignItems: "center" }}>
      ...
      {/* Overlay */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(170deg, rgba(8,14,20,0.82) 0%, rgba(8,14,20,0.55) 50%, rgba(8,14,20,0.90) 100%)", zIndex: 1 }} />
...
function LadiesClubGallerySection() {
  ...
  // Deep link: ?photo=YYYY-NN opens the lightbox straight to that photo, so a shared
  // link lands on the exact image instead of the top of the page. Runs once on mount.
  useEffect(() => {
    const m = new URLSearchParams(window.location.search).get("photo")?.match(/^(\d{4})-(\d+)$/);
    if (!m) return;
    const year = Number(m[1]);
    const idx = Number(m[2]) - 1;
    if (sets[year] && idx >= 0 && idx < sets[year].length) setLightbox({ year, index: idx });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Keep the URL in sync with the open photo so it stays shareable while swiping.
  useEffect(() => {
    if (lightbox) {
      const key = `${lightbox.year}-${String(lightbox.index + 1).padStart(2, "0")}`;
      window.history.replaceState(null, "", `${window.location.pathname}?photo=${key}`);
